...
const PackagingView: React.FC<{ orders?: ParsedOrder[] }> = ({ orders: propOrders }) => {
    ...
    const filteredResult = useMemo(() => {
        ...
        if (activeTab === 'Pending') {
            paged.forEach(order => {
                const dateStr = order.Timestamp ? new Date(order.Timestamp).toLocaleDateString('km-KH', { month: 'short', day: 'numeric' }) : 'ថ្មីៗ';
    ...
    const renderOrderCard = (order: ParsedOrder) => {
        ...
        const isUrgent = (new Date().getTime() - new Date(order.Timestamp).getTime()) > 86400000; // Over 24h

        return (
            <div key={order['Order ID']} className={`group relative bg-slate-900/40 backdrop-blur-xl border-2 ${isUrgent && activeTab === 'Pending' ? 'border-amber-500/30' : 'border-white/5'} rounded-[2.5rem] p-6 shadow-2xl transition-all duration-500 hover:scale-[1.02] hover:border-blue-500/40 hover:bg-slate-900/60 overflow-hidden`}>
                {isUrgent && activeTab === 'Pending' && <div className="absolute top-0 right-0 px-4 py-1 bg-amber-500 text-black text-[8px] font-black uppercase tracking-[0.2em] rounded-bl-2xl animate-pulse">ប្រញាប់ (Urgent)</div>}
                {loadingActionId === order['Order ID'] && <div className="absolute inset-0 bg-slate-950/80 z-50 flex items-center justify-center rounded-[2.5rem]"><Spinner /></div>}
                ...
                <div className="flex justify-between items-start mb-5">
                    <div className="min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                            ...
                            <span className="text-[10px] font-black text-blue-400 uppercase tracking-widest">{order.Team} Node</span>
                        </div>
                        <h3 className="text-white font-black text-xl truncate tracking-tight">{order['Customer Name']}</h3>
                        <p className="text-gray-400 font-mono text-sm font-bold mt-0.5">{order['Customer Phone']}</p>
                    </div>
                    <button onClick={() => setViewingOrder(order)} className="w-10 h-10 bg-white/5 hover:bg-white/10 text-gray-400 hover:text-white rounded-xl flex items-center justify-center transition-all border border-white/5 shadow-inner active:scale-90"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 5v.01M12 12v.01M12 19v.01M12 6a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2z" /></svg></button>
                </div>

                <div className="bg-black/30 rounded-2xl p-4 border border-white/5 mb-5 shadow-inner">
                    <div className="flex justify-between items-center mb-3">
                        <span className="text-[9px] font-black text-gray-500 uppercase tracking-widest">Inventory Assets</span>
                        <span className="text-[9px] font-black bg-blue-500/20 text-blue-400 px-2 py-0.5 rounded-md border border-blue-500/30">{order.Products.length} Items</span>
                    </div>
                    <div className="flex -space-x-3 overflow-hidden">
                        {order.Products.map((p, idx) => (
                            <img key={idx} src={convertGoogleDriveUrl(p.image)} className="w-12 h-12 rounded-xl object-cover border-2 border-[#0f172a] shadow-lg cursor-pointer hover:translate-y-[-4px] transition-transform" alt="" onClick={() => showFullImage(convertGoogleDriveUrl(p.image))} />
                        ))}
                    </div>
                </div>
                ...
                <div className="space-y-3 mb-6">
                    <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-lg bg-indigo-500/10 flex items-center justify-center text-indigo-400 border border-indigo-500/20"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" strokeWidth={2}/></svg></div>
                        <p className="text-xs font-bold text-gray-300 truncate">{order.Location}</p>
                    </div>
                    <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-lg bg-emerald-500/10 flex items-center justify-center text-emerald-400 border border-emerald-500/20"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" strokeWidth={2}/></svg></div>
                        <p className="text-sm font-black text-white font-mono">${(Number(order['Grand Total']) || 0).toFixed(2)}</p>
                    </div>
                </div>

                <div className="pt-4 border-t border-white/5">
                    {activeTab === 'Pending' && (
                        <button onClick={() => setPackingOrder(order)} className="w-full py-4 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white rounded-2xl font-black uppercase text-[11px] tracking-[0.2em] shadow-xl shadow-blue-900/40 transition-all active:scale-95 flex justify-center items-center gap-3 group/btn">
                            <svg className="w-5 h-5 group-hover/btn:scale-110 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={3}><path d="M5 13l4 4L19 7" /></svg>
                            ចាប់ផ្ដើមវេចខ្ចប់
                        </button>
                    )}
                    {activeTab === 'Ready to Ship' && (
                        <button onClick={() => executeAction(order, 'Shipped', { 'Dispatched Time': new Date().toLocaleString('km-KH'), 'Dispatched By': currentUser?.FullName || 'Packer' })} className="w-full py-4 bg-amber-600 hover:bg-amber-500 text-white rounded-2xl font-black uppercase text-[11px] tracking-[0.2em] shadow-xl shadow-amber-900/40 transition-all active:scale-95 flex justify-center items-center gap-3">
                            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={3}><path d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>
                            បញ្ជូនទៅអ្នកដឹក
                        </button>
                    )}
                </div>
    ...
    const renderOrderListRow = (order: ParsedOrder) => (
        <div key={order['Order ID']} className="bg-slate-900/40 backdrop-blur-xl border border-white/5 rounded-2xl p-4 flex items-center justify-between gap-6 hover:bg-slate-900/60 transition-all">
            <div className="flex items-center gap-4 min-w-0 flex-grow">
                <div className="w-12 h-12 rounded-xl bg-blue-600/20 flex items-center justify-center text-blue-400 border border-blue-500/20 font-black text-xs">{order.Team}</div>
                <div className="min-w-0">
                    <h4 className="text-white font-black truncate">{order['Customer Name']}</h4>
                    <p className="text-gray-500 text-xs font-mono">{order['Customer Phone']}</p>
                </div>
            </div>
            <div className="hidden md:block text-right">
                <p className="text-xs text-gray-500 font-black uppercase tracking-widest truncate max-w-[200px]">{order.Location}</p>
                <p className="text-emerald-400 font-mono font-black">${(Number(order['Grand Total']) || 0).toFixed(2)}</p>
            </div>
            <div className="flex gap-2">
                {activeTab === 'Pending' ? (
                    <button onClick={() => setPackingOrder(order)} className="px-6 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">Pack</button>
                ) : activeTab === 'Ready to Ship' ? (
                    <button onClick={() => executeAction(order, 'Shipped', { 'Dispatched Time': new Date().toLocaleString('km-KH'), 'Dispatched By': currentUser?.FullName || 'Packer' })} className="px-6 py-2 bg-amber-600 hover:bg-amber-500 text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">Ship</button>
                ) : (
                    <button onClick={() => setViewingOrder(order)} className="px-6 py-2 bg-white/5 hover:bg-white/10 text-gray-400 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">Details</button>
                )}
            </div>
        </div>
    );
    ...
    if (!selectedStore) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[80vh] p-6 animate-fade-in">
                <div className="w-full max-w-lg bg-slate-900/60 backdrop-blur-3xl border border-white/10 rounded-[3.5rem] p-10 shadow-3xl text-center space-y-10 relative overflow-hidden ring-1 ring-white/10">
                    <div className="absolute -top-40 -right-40 w-80 h-80 bg-blue-600/10 rounded-full blur-[100px] pointer-events-none"></div>
                    <div className="absolute -bottom-40 -left-40 w-80 h-80 bg-indigo-600/10 rounded-full blur-[100px] pointer-events-none"></div>
                    <div className="relative z-10 space-y-4">
                        <div className="w-24 h-24 bg-gradient-to-br from-blue-600 to-indigo-700 rounded-[2rem] mx-auto flex items-center justify-center border-2 border-white/20 shadow-2xl shadow-blue-900/40 transform -rotate-6 hover:rotate-0 transition-transform duration-500"><span className="text-5xl">🏬</span></div>
                        <h2 className="text-4xl font-black text-white uppercase tracking-tighter italic">Packaging Hub</h2>
                        <p className="text-xs text-gray-500 font-bold uppercase tracking-[0.4em]">Select Your Distribution Node</p>
                    </div>
                    <div className="relative z-10 grid grid-cols-1 gap-4">
                        {availableStores.map(store => (
                            <button key={store} onClick={() => setSelectedStore(store)} className="w-full py-5 px-8 bg-white/5 hover:bg-blue-600 text-white border border-white/5 rounded-2xl font-black text-base uppercase tracking-widest transition-all active:scale-95 flex items-center justify-between group shadow-xl">
                                <span>{store}</span><svg className="w-6 h-6 transform group-hover:translate-x-2 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={3}><path d="M13 7l5 5m0 0l-5 5m5-5H6" /></svg>
                            </button>
                        ))}
                    </div>
                </div>
                    ...
                    <div className="p-8 sm:p-12 bg-[#0f172a] rounded-[3rem] border border-white/10 shadow-3xl flex flex-col max-h-[90vh] relative overflow-hidden">
                        ...
                        <div className="flex-grow overflow-y-auto custom-scrollbar pr-4 space-y-10 relative z-10">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                ...
                                <div className="bg-white/[0.03] p-8 rounded-[2.5rem] border border-white/5 shadow-inner"><p className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.3em] mb-6">Logistics Flow</p><div className="space-y-6"><div className="flex justify-between items-center"><span className="text-[10px] text-gray-500 font-black uppercase tracking-widest">Team Node</span><span className="text-white font-black text-sm">{viewingOrder.Team}</span></div><div className="flex justify-between items-center"><span className="text-[10px] text-gray-500 font-black uppercase tracking-widest">Source Page</span><span className="text-white font-black text-xs">{viewingOrder.Page}</span></div><div className="flex justify-between items-center"><span className="text-[10px] text-gray-500 font-black uppercase tracking-widest">Fulfillment</span><span className="text-orange-400 font-black text-xs uppercase">{viewingOrder['Fulfillment Store']}</span></div><div className="flex justify-between items-center pt-4 border-t border-white/5"><span className="text-[10px] text-gray-500 font-black uppercase tracking-widest">Net Valuation</span><span className="text-emerald-400 font-black text-xl font-mono">${(Number(viewingOrder['Grand Total']) || 0).toFixed(2)}</span></div></div></div>
                            </div>
