  ...
  return (
    <div
      ...
      className={`relative rounded transition-all duration-75 ${
          ...
          isSelected 
            ? 'ring-2 ring-[#FCD535] ring-dashed bg-[#FCD535]/5 shadow-sm' 
            : ''
        } ${className}`}
      style={style}
    >
      ...
      {isSelected && (
        <>
            <div className="absolute -top-1 -left-1 w-2 h-2 bg-[#FCD535] border border-white rounded-full"></div>
            <div className="absolute -bottom-1 -right-1 w-2 h-2 bg-[#FCD535] border border-white rounded-full"></div>
        </>
      )}
    </div>
  );
...
export const SmartQR: React.FC<{ value: string, baseSize: number, isDesignMode: boolean, storageKey?: string }> = ({ value, baseSize, isDesignMode, storageKey }) => {
    ...
    return (
      <div 
        ...
        className={`relative ${isDesignMode && !isSelected ? "hover:outline hover:outline-1 hover:outline-black/20 rounded" : ""} ${isSelected ? "ring-2 ring-[#FCD535] ring-dashed bg-[#FCD535]/5 p-1 rounded" : ""}`}
      >
