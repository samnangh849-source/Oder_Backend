    ...
    return (
        <div className="space-y-3">
            <h3 className={`text-sm font-bold ${isLightMode ? 'text-slate-400' : 'text-slate-500'} flex items-center px-1 uppercase tracking-wider`}>
                <i className="fa-solid fa-warehouse mr-2 text-orange-500 opacity-70"></i>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left border-collapse">
                        ...
                        <tbody className={`divide-y ${isLightMode ? 'divide-slate-100' : 'divide-white/[0.04]'}`}>
                            {stats.map((store, idx) => (
                                <tr key={store.name} className={`${isLightMode ? 'hover:bg-slate-50' : 'even:bg-white/[0.02] hover:bg-white/[0.05]'} transition-colors group cursor-pointer`} onClick={() => onStatClick(store.name)}>
                                    <td className="px-4 py-3 flex items-center gap-3">
                                        ...
                                        <span className={`font-bold ${isLightMode ? 'text-slate-600' : 'text-slate-200'}`}>{store.name}</span>
                                    </td>
